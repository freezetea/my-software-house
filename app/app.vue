    ...
    <Transition name="loader">
      <div v-if="loading" class="fixed inset-0 z-[999] flex flex-col items-center justify-center" style="background: linear-gradient(135deg, #0a0d3d 0%, #0f1147 60%, #1a1060 100%)">
        ...
        <div class="flex items-center gap-3 mb-8 relative z-10">
          <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 flex items-center justify-center shadow-xl shadow-indigo-500/40">
            <span class="text-white font-black text-xl">K</span>
          </div>
          <div class="text-white font-black text-3xl tracking-tight">
            koda<span class="text-indigo-300">kode</span>
          </div>
        </div>
        ...
        <div class="w-48 h-1 bg-white/10 rounded-full overflow-hidden relative z-10">
          <div class="h-full bg-gradient-to-r from-indigo-500 via-violet-500 to-indigo-500 rounded-full transition-all duration-300"
        ...
        <div class="flex gap-2 mt-6 relative z-10">
          <div v-for="i in 3" :key="i"
            ...
            :style="`animation-delay: ${(i-1) * 0.15}s`"></div>
        </div>
      </div>
    </Transition>
      ...
      <div class="max-w-6xl mx-auto px-4 md:px-6 py-14 grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div>
          ...
          <ul class="flex flex-col gap-2.5">
            <li v-for="s in footerServices" :key="s"><a href="#services" class="text-slate-400 hover:text-indigo-300 text-sm transition-colors">{{ s }}</a></li>
          </ul>
        </div>
